...
const compressCirclesToString = function(arr) {
	let out = arr.map(b => b.map(c => parseInt(c * 1000 + 500).toString(36).padStart(2, '0')))
	out = out.map(b => b.join('')).join('')
	return out
}
...
function draw() {	
	...
	function postProcess(circles) {
		...
		console.log('\n', compressCirclesToString(normalizeCircles(bestCircles, img)))
	}
