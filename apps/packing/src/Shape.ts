...
export default class Shape {
  ...
  constructor(type:string | number[][]) {
    if (type) {
      ...
      if (Array.isArray(type)) {
        this.fromArray(type as ArrayCircle[])
      }
    }
  }

  fromArray(arr:ArrayCircle[], scale:number=1) {
  ...
  scaleRotateTranslate(scale:number, rotateRadians:number, translateX:number, translateY:number) {
    this.circles = []
  ...
  fromString(str:string) {
    const out = str.split(/(.{2})/)
      .filter(_ => _.length === 2)
      .map(n => parseFloat((parseInt(n, 36) / 1000 - 0.5).toFixed(3)))
    const outA = []
